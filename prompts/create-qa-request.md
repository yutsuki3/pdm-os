...
3. テスト対象・対象外・リスクは入力に根拠があるものだけを記載する。不明な検証環境・entry/exit criteria・送付先チャンネルは `TBD` とする。
4. 送付は行わない。Slackの送付パーマリンクとJiraのQAチケットは、人間が実際に送付・起票した後にのみ記入される。草稿段階では未送付と明記する。
