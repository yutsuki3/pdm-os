...
3. 各タスク候補を [templates/jira-task.md](../templates/jira-task.md) 形式で記述し、Jira登録前に一意な `draft_id` を付与する。各受け入れ条件には、`source_type` / `source_ref` / `item_ref` の組で根拠を記載する。仕様書要件は `specification` / 仕様書ID / `FR-001` 等、画面は `wireflow` / ワイヤーフロー参照 / `S2` 等とする。
