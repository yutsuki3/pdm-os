...
1. ユーザーは通知設定画面で、通知種別ごとにON/OFFを切り替えられる
2. 設定変更は即時反映される（反映タイミングの厳密な定義はTBD）
3. デフォルト設定は既存ユーザーの現行設定（全ON相当）を引き継ぐ
...
TBD — templates/wireflow.md によるワイヤーフロー未作成（サンプルのため省略）。
